...
// #######################################################################
if ($_POST['do'] == 'dotransfer' OR $_POST['do'] == 'transfer')
{
...
if ($_POST['do'] == 'dotransfer')
{
	...
	else
	{
		...
		$pmdm->setr('message', construct_phrase($vbphrase['pm_request_to_take_ownership_of_social_group_message'], $vbulletin->options['bburl'], $group['groupid'], fetch_word_wrapped_string(fetch_censored_text($group['name']))));
		...
		)
		{
			foreach ($vbulletin->bookmarksitecache AS $bookmarksite)
			{
				$bookmarksite['link'] = str_replace(
					array('{URL}', '{TITLE}'),
					array(urlencode($vbulletin->options['bburl'] . '/group.php?discussionid=' . $discussion['discussionid']), urlencode($discussion['title'])),
					$bookmarksite['url']
				);
