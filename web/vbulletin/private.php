...
if ($_REQUEST['do'] == 'trackpm')
{
	...
	if (!$vbulletin->GPC['type'] OR $vbulletin->GPC['type'] == 'confirmed')
	{
		...
		{
			$show['readpm'] = true;
			$numreceipts = vb_number_format($readtotal);
	...
	if (!$vbulletin->GPC['type'] OR $vbulletin->GPC['type'] == 'unconfirmed')
	{
		...
		{
			$show['readpm'] = false;
			$numreceipts = vb_number_format($unreadtotal);
...
if ($_REQUEST['do'] == 'showpm')
{
	...
	// report pm icon?
	$show['reportlink'] = (
		($vbulletin->options['rpforumid'] OR
		($vbulletin->options['enableemail'] AND $vbulletin->options['rpemail'])) AND
		($vbulletin->userinfo['userid'] != $pm['fromuserid'])
	);
...
if ($_REQUEST['do'] == 'showhistory')
{
	...
	else
	{
		...
		{
			eval('$threadpms = "' . fetch_template('pm_nomessagehistory') . '";');
		}
