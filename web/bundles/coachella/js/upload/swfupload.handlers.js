    ...
    try {
		if(fileContor == 0){
			...
			if(typeof(serverDataJSON.message) != 'undefined' && typeof(serverDataJSON.message_info) != 'undefined'  && serverDataJSON.message_type == 'error'){
				...
				addImage("/bundles/coachella/upload_images/" + serverDataJSON.file_name);
		...
		$.ajax({
			...
			success: function(msg){
				...
				if(msgJSON && typeof(msgJSON.response) != 'undefined' && msgJSON.response != 'ok'){
					...
					location.href = "/bundles/uploads/" + msgJSON.file_name;
				}
				fileContor--;
			}
		});
...
var jsUploadPath 	= '/bundles/uploads/';
...
function addImage(src) {
	...
	document.getElementById("thumbnail").appendChild(newImg);
	if (newImg.filters) {
		try {
	...
	//pt CROP
	$('#thumbnail').imgAreaSelect({aspectRatio: '1:1', 
                                    handles: true,
                                    fadeSpeed: '200',
	...
	htmlForCrop += '<form name="thumbnail" action="" method="post">';
