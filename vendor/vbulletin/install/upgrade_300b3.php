...
if ($vbulletin->GPC['step'] == 10)
{
	...
	{
		echo "<li>" . sprintf($upgrade_phrases['upgrade_300b3.php']['announcement_x'], htmlspecialchars($an['title'])) . " ... \n";
...
if ($vbulletin->GPC['step'] == 13)
{
	...
	$attachments = $db->query_read("
		...
		" . iif(version_compare(MYSQL_VERSION, '3.23.2', '<'), ', attachment.filedata', '') . "
		FROM post
	...
	while ($thisattach = $db->fetch_array($attachments))
	{
		...
		else
		{
			...
			$db->query_write("
				...
				filehash = " . iif(version_compare(MYSQL_VERSION, '3.23.2', '<'), "'" . $db->escape_string(md5($thisattach['filedata'])) . "'",  'MD5(filedata)') . "
			WHERE attachmentid = $thisattach[attachmentid]");
			echo "{$vbphrase['done']}</li>\n";
		}
		vbflush();
...
if ($vbulletin->GPC['step'] == 18)
{
	...
	$db->query_write("ALTER IGNORE TABLE subscribethread ADD UNIQUE subscribeindex (userid, threadid)");
...
if ($vbulletin->GPC['step'] == 28)
{
	...
	//xml_import_language(false, -1, '', 0, 1, false);

	$query[] = "ALTER TABLE " . TABLE_PREFIX . "language TYPE = MYISAM";
	...
	{
		echo "<li>" . sprintf($upgrade_phrases['upgrade_300b3.php']['building_lang_x'], $language['title']) . "</li>";
		build_language($languageid);
		vbflush();
	}
	echo "</ul>\n";
	echo "<p><b>{$upgrade_phrases['upgrade_300b3.php']['language_imported_sucessfully']}</b></p>";*/
}
...
if ($vbulletin->GPC['step'] == 33)
{
	...
	$styleQuery = array(
		"('" . sprintf($upgrade_phrases['upgrade_300b3.php']['bbtitle_vb3_style'], $db->escape_string(htmlspecialchars($bbtitle))) . "', 2, '', '', '', 1, 1)",
		"('vBulletin 3', -1, '', '', '', 0, 1)"
	);
