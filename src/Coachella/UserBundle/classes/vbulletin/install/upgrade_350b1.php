...
if ($vbulletin->GPC['step'] == 2)
{
	...
	$upgrade->run_query(
		...
		"ALTER IGNORE TABLE " . TABLE_PREFIX . "useractivation ADD UNIQUE INDEX userid (userid, type)",
		MYSQL_ERROR_KEY_EXISTS
	);
	$upgrade->run_query(
		sprintf($upgradecore_phrases['altering_x_table'], 'usergrouprequest', 1, 1),
		"ALTER IGNORE TABLE " . TABLE_PREFIX . "usergrouprequest ADD UNIQUE INDEX userid (userid, usergroupid)",
		MYSQL_ERROR_KEY_EXISTS
	);
...
if ($vbulletin->GPC['step'] == 3)
{
	...
	$upgrade->run_query(
		'',
		"ALTER IGNORE TABLE " . TABLE_PREFIX . "adminutil ADD PRIMARY KEY (title)",
...
if ($vbulletin->GPC['step'] == 4)
{
	...
	$upgrade->run_query(
		...
		"ALTER IGNORE TABLE " . TABLE_PREFIX . "subscribeevent ADD UNIQUE INDEX subindex (userid, eventid)",
		MYSQL_ERROR_KEY_EXISTS
	);
