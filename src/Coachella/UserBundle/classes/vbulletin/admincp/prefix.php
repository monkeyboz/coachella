...
define('CVS_REVISION', '$RCSfile$ - $Revision: 31504 $');
...
if ($_REQUEST['do'] == 'permissions')
{
	...
	else
	{
		$conflict_options_default = array(
			...
			'0'  => $vbphrase['new_groups_may_use_selected_prefixes'],
			'1'  => $vbphrase['new_groups_may_not_use_selected_prefixes']
		);

		print_label_row(
			$vbphrase['allow_new_groups_to_use_selected_prefixes'],
			"<label for=\"sel_ug$usergroupid\" class=\"smallfont\">" . $vbphrase['set_default_permissions'] . ": <select name=\"conflict[$usergroupid]\" id=\"sel_ug$usergroupid\">" . construct_select_options($conflict_options_default, '-1') . "</select>"
		);
	}
...
if ($_POST['do'] == 'savepermissions')
{
	...
	{
		$vbulletin->db->query_write("
			UPDATE " . TABLE_PREFIX . "prefix SET options = options " . ($vbulletin->GPC['default'] == 1 ? '| ' : '& ~') . $vbulletin->bf_misc_prefixoptions['deny_by_default'] . "
			WHERE prefixid IN ('" . implode("', '", array_map(array($vbulletin->db, 'escape_string'), $prefixids)) . "')
		");
	}
