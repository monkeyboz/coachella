...
class vB_Collection_Factory
{
	 ...
	 * @param boolean $descending				Whether to get results in descending order
	 * @param boolean $no_limit					Whether to get all results or not
	 * @return vB_Collection				The appropriate collection
	 */
 ...
 * @see vB_Collection::fetch_counts()
 *
 * @package		vBulletin
...
class vB_Collection
{
	 ...
	 */
	function vB_Collection(&$registry, $parent_id = false, $page = 1, $quantity = false, $descending = true, $no_limit = false)
	{
		...
		{
			trigger_error("vB_Collection::Registry object is not an object", E_USER_ERROR);
		}
 ...
 * @abstract
 */
class vB_Group_Collection extends vB_Collection
{
	/**
	 * Information about the social group.
	 *
	...
	{
		parent::vB_Collection($registry, $parent_id, $page, $quantity, $descending, $no_limit);
 ...
 * @copyright 	http://www.vbulletin.com/license.html
 */
class vB_Collection_Album extends vB_Collection
{
	/**
	 * Type identifier.
	 * Given to items so it can be checked whenever they are processed.
	...
	function collection_sql($force_refresh = false)
	{
		...
		$sql .= "	LEFT JOIN " . TABLE_PREFIX . "picture AS picture ON (album.coverpictureid = picture.pictureid AND picture.thumbnail_filesize > 0)
					$hook_query_joins
					WHERE 1=1";
 ...
 * @copyright 	http://www.vbulletin.com/license.html
 */
class vB_Collection_GroupCategory extends vB_Collection
{
	/**
	 * Type identifier.
	 * Given to items so it can be checked whenever they are processed.
	...
	*/
	...
	function process_display()
	{
		...
		if ($this->show_moderation_tools)
		{
			if ($this->item['is_discussion'])
			{
				$this->item['edit'] = (can_edit_group_discussion($discussion) OR can_edit_group_message($this->item, $this->info['group']));
			}
			else
			{
				$this->item['edit'] = can_edit_group_message($this->item, $this->info['group']);
			}
		}
		else
		{
